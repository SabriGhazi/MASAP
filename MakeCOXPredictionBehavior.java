...
public class MakeCOXPredictionBehavior extends TickerBehaviour{
	...
	MakeCOXPredictionBehavior(Agent agnt){
		super(agnt,10);
		_agnt=agnt;
	}
	@Override
		...
		if(SimulationFramework.getCurrentSimProcessState()== SimProcessState.Started){
			...
			if(paramsInfoMsg!=null){
				...
				if(nbrOfMsg==3){
					...
					double currentValOfPM10=SimulationFramework.getCurrentEnv().COx.getData().get(t);
					...
					SimulationFramework.getCurrentEnv().sim_COx.addData(d);
					msg="\n"+_agnt.getName()+" : <COX Concentration is> : "+d;
					msg=" ["+ msg+hu+" | "+temp+" | "+ws+"]";
					//Console.logMSG(msg);
